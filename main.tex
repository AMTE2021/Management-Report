...
\fancyhead[c]{AMT21 - Management report}
...
\author{Patrick Diehl \\ Irina Demeshko \\ Zahra Khatami \\ Steven R. Brandt \\ Parsa Amini}
\date{\today}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
The first submission deadline was May 7th with an extension to May 14th. We received three full papers at the first extension. Note that the workshop does not accept short papers. The deadline was extended to June 14th, and we received eight submissions at the final deadline. In total, eight papers were submitted on June 14th and five papers were accepted ($62.5$\%). The workshop was scheduled at August 30th, and we had an average attendance of 10 persons for the talks. The keynote was given by Thomas Fahringer (University of Innsbruck, Austria) and 12 persons attended the keynote. The invited talks was given by Daisy Hollman (Google) and 18 persons attended. The workshops closed with a panel discussion, and 14 persons attended the panel discussion. More details are available on the workshop's webpage\footnote{https://amte2021.stellar-group.org/}.
...
\begin{itemize}
    ...
    \item 10:20am - 10:50am, Accelerating Messages by Avoiding Copies in an Asynchronous Task-based Programming Model 
    \item 10:50am - 11:20am, An Experimental Study of SYCL Task Graph Parallelism for Large-Scale Machine Learning Workloads
    \item 11:20am - 11:50am, Combining OpenMP tasking and target (GPU) offloading for productivity and performance on heterogeneous systems
    \item 11:50am - 12:20pm, Understanding the Effect of Task Granularity on Execution Time in Asynchronous Many-Task Runtime Systems
    \item 12:20pm - 1:30pm, Lunch Break
    \item 1:30pm - 2:30pm, Invited talk (Daisy S. Hollman, Sandia National Laboratory, USA)
    \item 2:30pm - 3:00pm, FleCSI 2.0: The Flexible Computational Science Infrastructure Project
    \item 3:30pm - 4:30pm, Panel
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Many languages have added “await” or some variants as new keywords for processing asynchronous code. How important is this feature for the success of the AMT paradigm?
    \end{itemize}
    \item
...
\begin{enumerate}
    ...
    \begin{center}
        \textbf{Abstract}
    \end{center}
    The FleCSI 2.0 programming system supports multiphysics application development through a runtime abstraction layer, and by providing core topology types that can be customized for specifc numerical methods. The abstraction layer provides a single-source programming interface for distributed and shared-memory data parallelism through task and kernel execution, and has been demonstrated to introduce virtually no runtime overhead. FleCSI’s core topology types represent a rich set of basic data structures that can be specialized to create application-facing interfaces for a variety of different physics packages. Using the FleCSI control and data models, it is straightforward to compose multiple packages to create full multiphysics applications. When used with the Legion backend, FleCSI offers extended runtime analysis that can increase task concurrency, facilitate load balancing, and allow for portability across heterogeneous computing architectures.
